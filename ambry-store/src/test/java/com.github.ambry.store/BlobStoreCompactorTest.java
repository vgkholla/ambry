...
public class BlobStoreCompactorTest {
  ...
  public void badInputTest() throws Exception {
    ...
    // compaction range contains log segments that are last modified past the reference time.
    details = new CompactionDetails(0, Collections.singletonList(state.log.getFirstSegment().getName()));
    ensureArgumentFailure(details,
        "Should have failed because compaction range contains log segments modified after reference time");
  }
  ...
  private void addToLogEntriesInOrder(List<IndexEntry> indexEntries, List<LogEntry> logEntriesInOrder) {
    Collections.sort(indexEntries, PersistentIndex.INDEX_ENTRIES_COMPARATOR);
