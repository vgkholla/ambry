...
class PersistentIndex {
  ...
  PersistentIndex(String datadir, String storeId, ScheduledExecutorService scheduler, Log log, StoreConfig config,
      ...
      throws StoreException {
    /*
    The journal holds all operations on a key (put, update) whereas the latest index segment has only one entry per
    key. The journal size is twice the size of the index segment to account for these "extra" entries. However, a
    rollover will occur to maintain the invariant that all entries in the latest segment are also in the journal.
    */
  ...
  PersistentIndex(String datadir, String storeId, ScheduledExecutorService scheduler, Log log, StoreConfig config,
    ...
    try {
      ...
      journal.setLatestIndexSegmentStartOffset(validIndexSegments.lastKey());
  ...
  private void recover(MessageStoreRecovery recovery) throws StoreException, IOException {
    // TODO (TTL update): handle TTL update in recovery (needs changes in MessageInfo)
   ...
   * @throws StoreException
   */
  IndexValue addToIndex(IndexEntry entry, FileSpan fileSpan) throws StoreException {
    IndexValue value;
    ...
    if (needToRollOverIndex(entry)) {
      ...
      value = info.addEntry(entry, fileSpan.getEndOffset());
      ...
      value = validIndexSegments.lastEntry().getValue().addEntry(entry, fileSpan.getEndOffset());
    }
  ...
  private boolean needToRollOverIndex(IndexEntry entry) {
    ...
    // if the journal's first entry is within the current index segment and the journal is full, then rollover to
    // maintain the invariant that all entries in the current index segment must be in the journal
    if (journal.isFull() && journal.getFirstOffset().compareTo(lastSegment.getStartOffset()) >= 0) {
      logger.info("Index: {} Rolling over from {} to {} because the journal is full and can no longer have more entries"
          + " from the current index segment", dataDir, lastSegment.getStartOffset(), offset);
    }
    return false;
  }
  ...
  private IndexValue findKey(StoreKey key, FileSpan fileSpan, EnumSet<IndexEntryType> types,
    ...
    List<IndexValue> foundValues = new ArrayList<>();
    ...
    try {
      ...
      for (Map.Entry<Offset, IndexSegment> entry : segmentsMapToSearch.entrySet()) {
        ...
        if (value != null) {
          ...
          if (types.contains(IndexEntryType.DELETE) && value.isFlagSet(IndexValue.Flags.Delete_Index)) {
            ...
            long relatedMessageOffset = value.getRelatedMessageOffset();
            if (!value.isFlagSet(IndexValue.Flags.Ttl_Update_Index) || (
                relatedMessageOffset != IndexValue.UNKNOWN_RELATED_MESSAGE_OFFSET
                    && relatedMessageOffset > value.getOffset().getOffset())) {
              // this is not a ttl update entry OR
              // this is a merged put and ttl update index entry
              retValue = value;
              break;
            }
          }
      ...
      if (retValue != null) {
        ...
        if (latest.getExpiresAtMs() != retValue.getExpiresAtMs()) {
          ...
          retValue.setExpiresAtMs(foundValues.get(0).getExpiresAtMs());
        }
      }
  ...
  private IndexValue markAsDeleted(StoreKey id, FileSpan fileSpan, MessageInfo info, long deletionTimeMs)
    ...
    return addToIndex(new IndexEntry(id, newValue, null), fileSpan);
  }
  ...
  IndexValue markAsPermanent(StoreKey id, FileSpan fileSpan, long operationTimeMs) throws StoreException {
    ...
    if (value == null) {
      ...
      throw new StoreException("TTL of Id " + id + " already updated in index" + dataDir,
          StoreErrorCodes.TTL_Already_Updated);
    }
    long size = fileSpan.getEndOffset().getOffset() - fileSpan.getStartOffset().getOffset();
    IndexValue newValue =
        new IndexValue(value.getSize(), value.getOffset(), Utils.Infinite_Time, operationTimeMs, value.getAccountId(),
            value.getContainerId());
    newValue.setFlag(IndexValue.Flags.Ttl_Update_Index);
    newValue.setNewOffset(fileSpan.getStartOffset());
    newValue.setNewSize(size);
    newValue = addToIndex(new IndexEntry(id, newValue, null), fileSpan);
    return new IndexValue(newValue.getOffset().getName(), newValue.getBytes(), newValue.getVersion());
  }
    ...
    try {
      IndexValue putValue =
          findKey(key, new FileSpan(getStartOffset(indexSegments), value.getOffset()), EnumSet.of(IndexEntryType.PUT),
              indexSegments);
      if (value.getRelatedMessageOffset() != value.getOffset().getOffset()
          && value.getRelatedMessageOffset() != IndexValue.UNKNOWN_RELATED_MESSAGE_OFFSET) {
        ...
        MessageInfo deletedBlobInfo =
            hardDelete.getMessageInfo(log.getSegment(logSegmentName), value.getRelatedMessageOffset(), factory);
        if (putValue != null && putValue.getOffset().getName().equals(value.getOffset().getName())) {
          if (putValue.getOffset().getOffset() != value.getRelatedMessageOffset()) {
            logger.error(
                "Offset in PUT index entry {} is different from original message offset in delete entry {} for key {}",
                putValue.getOffset().getOffset(), value.getRelatedMessageOffset(), key);
        ...
        Offset offset = new Offset(logSegmentName, value.getRelatedMessageOffset());
        ...
        readOptions = new BlobReadOptions(log, offset,
            new MessageInfo(deletedBlobInfo.getStoreKey(), deletedBlobInfo.getSize(), value.getExpiresAtMs(),
        ...
        readOptions = new BlobReadOptions(log, putValue.getOffset(),
            new MessageInfo(key, putValue.getSize(), value.getExpiresAtMs(), putValue.getAccountId(),
        ...
        throw new StoreException("Did not find PUT index entry for key [" + key
            + "] and the the original offset in value of the DELETE entry was [" + value.getRelatedMessageOffset()
  ...
  private void eliminateDuplicates(List<MessageInfo> messageEntries) {
    ...
    ListIterator<MessageInfo> messageEntriesIterator = messageEntries.listIterator(0);
    ...
    messageEntriesIterator = messageEntries.listIterator(messageEntries.size());
    ...
    while (messageEntriesIterator.hasNext()) {
      ...
      if (!messageInfo.isDeleted()) {
        ...
        messageInfo = new MessageInfo(messageInfo.getStoreKey(), messageInfo.getSize(),
            indexValue.isFlagSet(IndexValue.Flags.Delete_Index), indexValue.getExpiresAtMs(), indexValue.getAccountId(),
            indexValue.getContainerId(), indexValue.getOperationTimeInMs());
