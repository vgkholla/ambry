...
class BlobStoreCompactor {
  ...
  private void checkSanity(CompactionDetails details) {
    ...
    IndexSegment lastIndexSegment = srcIndex.getIndexSegments().floorEntry(lastSegmentEndOffset).getValue();
    long lastModifiedTimeMs = lastIndexSegment.getLastModifiedTimeMs();
    if (lastModifiedTimeMs >= details.getReferenceTimeMs()) {
      throw new IllegalArgumentException(
          "Compaction range contains some log segments whose last modified time is " + lastModifiedTimeMs
              + " which is equal or higher than the reference time " + details.getReferenceTimeMs());
    }
  }
  ...
  private void copy() throws InterruptedException, IOException, StoreException {
    ...
    Offset latestEligibleOffsetForDelete = getLatestEligibleOffsetForDelete();
   ...
   */
  private Offset getLatestEligibleOffsetForDelete() {
    // TODO: move this to BlobStoreStats
    long referenceTimeMs = compactionLog.getCompactionDetails().getReferenceTimeMs();
    Offset latestEligibleOffset = srcIndex.getIndexSegments().lastKey();
    for (IndexSegment indexSegment : srcIndex.getIndexSegments().descendingMap().values()) {
      if (indexSegment.getLastModifiedTimeMs() < referenceTimeMs) {
        latestEligibleOffset = indexSegment.getEndOffset();
        break;
      }
    }
   ...
   * @param latestEligibleOffsetForDelete the {@link Offset} up until which delete records are considered applicable.
  ...
  private boolean copyLogSegmentData(LogSegment from, FileSpan duplicateSearchSpan,
      Offset latestEligibleOffsetForDelete) throws IOException, StoreException {
    boolean allCopied = true;
    for (Offset indexSegmentStartOffset : getIndexSegmentDetails(from.getName()).keySet()) {
      FileSpan forwardSearchSpan = new FileSpan(indexSegmentStartOffset, latestEligibleOffsetForDelete);
   ...
   * @param duplicateSearchSpan the {@link FileSpan} in which to search for duplicates.
   * @param forwardSearchSpan the {@link FileSpan} in which to search for delete records.
  ...
  private boolean copyIndexSegmentData(LogSegment from, IndexSegment indexSegmentToCopy, FileSpan duplicateSearchSpan,
      FileSpan forwardSearchSpan) throws IOException, StoreException {
    ...
    // get all entries
    indexSegmentToCopy.getEntriesSince(null, new FindEntriesCondition(Long.MAX_VALUE), messageInfos, new AtomicLong(0));

    // save a token for restart (the key gets ignored but is required to be non null for construction)
    StoreFindToken safeToken =
        new StoreFindToken(messageInfos.get(0).getStoreKey(), indexSegmentToCopy.getStartOffset(), sessionId,
            incarnationId);
    compactionLog.setSafeToken(safeToken);

    boolean checkAlreadyExists =
    ...
    List<IndexEntry> indexEntries =
        getValidIndexEntries(from, messageInfos, duplicateSearchSpan, forwardSearchSpan, checkAlreadyExists);

    // Copy these over
    boolean copiedAll = copyRecords(from, indexEntries, indexSegmentToCopy.getLastModifiedTimeSecs());
    // persist
   ...
   * @param from the log segment to copy from.
   * @param messageInfos the {@link MessageInfo} of all the entries in the index segment.
   * @param duplicateSearchSpan the {@link FileSpan} in which to search for duplicates.
   * @param forwardSearchSpan the {@link FileSpan} in which to search for delete records.
   * @param checkAlreadyExists {@code true} if a check for existence in the swap spaces has to be executed (due to
   ...
   */
  private List<IndexEntry> getValidIndexEntries(LogSegment from, List<MessageInfo> messageInfos,
      FileSpan duplicateSearchSpan, FileSpan forwardSearchSpan, boolean checkAlreadyExists) throws StoreException {
    // TODO: move this blob store stats i.e. get all valid entries and then do the duplicate searching.
    List<IndexEntry> indexEntries = new ArrayList<>();
    for (MessageInfo info : messageInfos) {
      if (info.isDeleted() || !isExpired(info)) {
        // either a DELETE record or an unexpired PUT record.
        if (duplicateSearchSpan != null) {
          // make sure this is not a duplicated record
          IndexValue possibleDuplicate = srcIndex.findKey(info.getStoreKey(), duplicateSearchSpan);
          if (possibleDuplicate != null && (!info.isDeleted() || possibleDuplicate.isFlagSet(
              IndexValue.Flags.Delete_Index))) {
            // An index value was found AND
            // Info under consideration is a PUT record (therefore whatever we have now is a duplicate for sure)
            // Info under consideration is a DELETE record and so is the index value that was found
            // In both cases, we have just encountered a duplicate record. Continue.
            logger.trace("Skipping {} because it is a duplicate entry", info);
            continue;
          }
        }
        IndexValue value = srcIndex.findKey(info.getStoreKey(), forwardSearchSpan);
        if (info.isDeleted() || !value.isFlagSet(IndexValue.Flags.Delete_Index)) {
          // 1. DELETE record
          // 2. PUT record that has not expired nor has been deleted within the referenceTime
          if (info.isDeleted() && !value.isFlagSet(IndexValue.Flags.Delete_Index)) {
            // should have got the DELETE entry
            throw new IllegalStateException(
                "Did not get the DELETE index entry corresponding to the delete record that " + "needed to be copied - "
                    + info);
          } else if (!value.getOffset().getName().equals(from.getName())) {
            // offset obtained should be sane.
            throw new IllegalStateException(
                "Value obtained while searching for info [" + info + "] has offset " + value.getOffset()
                    + " which does not lie in the log segment being copied " + from.getName());
          } else if (!checkAlreadyExists || !alreadyExistsInTgt(info.getStoreKey(), value)) {
            logger.trace("Adding index entry for {} because it is valid and does not already exist", info);
            indexEntries.add(new IndexEntry(info.getStoreKey(), value));
          }
        } else {
          logger.trace("Skipping {} because it is a deleted PUT", info);
        }
      } else {
        logger.trace("Skipping {} because it is an expired PUT", info);
      }
    }
  ...
  /**
   * Determines if a message is expired at the given {@code referenceTime}.
   * @param info the {@link MessageInfo} of the message to check.
   * @return {@code true} if expired, {@code false} if not.
   */
  private boolean isExpired(MessageInfo info) {
    // TODO: move this to PersistentIndex/MessageInfo
    return info.getExpirationTimeInMs() != Utils.Infinite_Time && time.milliseconds() > info.getExpirationTimeInMs();
  }
