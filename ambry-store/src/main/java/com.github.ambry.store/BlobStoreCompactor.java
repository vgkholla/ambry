...
class BlobStoreCompactor {
  ...
  private boolean copyDataByIndexSegment(LogSegment logSegmentToCopy, IndexSegment indexSegmentToCopy,
    ...
    List<IndexEntry> indexEntriesToCopy =
        getIndexEntriesToCopy(allIndexEntries, duplicateSearchSpan, indexSegmentToCopy, checkAlreadyCopied);
  ...
  private List<IndexEntry> getIndexEntriesToCopy(List<IndexEntry> allIndexEntries, FileSpan duplicateSearchSpan,
      IndexSegment indexSegment, boolean checkAlreadyCopied) throws StoreException {
  ...
  /**
   * Deterimines if {@code copuCandidate} is a duplicate.
   ...
   * @param checkAlreadyCopied if {@code true}, checks if {@code copyCanidate} has already been copied.
  ...
  private List<IndexEntry> getValidIndexEntries(IndexSegment indexSegment, List<IndexEntry> allIndexEntries)
    ...
    for (IndexEntry indexEntry : allIndexEntries) {
      ...
      if (value.isFlagSet(IndexValue.Flags.Delete_Index)) {
        ...
        // no PUT or DELETE in the same index segment. This entry is "valid" if the corresponding PUT is still alive
        // The PUT entry, if it exists, must be "before" this TTL update entry.
        FileSpan srcSearchSpan = new FileSpan(srcIndex.getStartOffset(), indexSegment.getStartOffset());
        IndexValue srcValue =
            srcIndex.findKey(indexEntry.getKey(), srcSearchSpan, EnumSet.of(PersistentIndex.IndexEntryType.PUT));
        if (srcValue == null) {
          // PUT is not in the source - therefore can't be in target. This TTL update can be cleaned up
          logger.trace("Skipping copying {} in segment with start offset {} in {} because the corresponding PUT entry "
              + "does not exist anymore", indexEntry, indexSegment.getStartOffset(), storeId);
        } else {
          IndexValue tgtValue =
              tgtIndex.findKey(indexEntry.getKey(), null, EnumSet.of(PersistentIndex.IndexEntryType.PUT));
          if (tgtValue == null && isOffsetInCurrentCycle(srcValue.getOffset())) {
            ...
            logger.trace("Skipping copying {} in segment with start offset {} in {} because the corresponding PUT entry"
                    + " {} will be compacted in this cycle ({} are being compacted in this cycle)", indexEntry,
                indexSegment.getStartOffset(), storeId, srcValue,
  ...
  private boolean isOffsetInCurrentCycle(Offset offset) {
    ...
    return offset.compareTo(start) >= 0 && offset.compareTo(end) <= 0;
  }
