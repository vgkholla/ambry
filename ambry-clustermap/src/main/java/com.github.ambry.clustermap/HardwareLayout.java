...
class HardwareLayout {
  ...
  private final Map<Byte, Datacenter> datacentersById;
  ...
  public HardwareLayout(JSONObject jsonObject, ClusterMapConfig clusterMapConfig) throws JSONException {
    ...
    for (int i = 0; i < jsonObject.getJSONArray("datacenters").length(); ++i) {
      ...
      this.datacenters.add(i, datacenter);
      datacentersById.put(datacenter.getId(), datacenter);
    }
  ...
  Datacenter getDatacenterById(byte id) {
    return datacentersById.get(id);
  }
