  ...
  public void handleTtlUpdateRequest(Request request) throws IOException, InterruptedException {
    ...
    try {
      ...
      if (error != ServerErrorCode.No_Error) {
        ...
        StoreKey convertedStoreKey = getConvertedStoreKeys(Collections.singletonList(updateRequest.getBlobId())).get(0);
        MessageFormatInputStream stream =
            new TtlUpdateMessageFormatInputStream(convertedStoreKey, updateRequest.getAccountId(),
                updateRequest.getContainerId(), updateRequest.getExpiresAtMs(), updateRequest.getOperationTimeInMs());
        MessageInfo info =
            new MessageInfo(convertedStoreKey, stream.getSize(), false, true, updateRequest.getExpiresAtMs(),
                updateRequest.getAccountId(), updateRequest.getContainerId(), updateRequest.getOperationTimeInMs());
