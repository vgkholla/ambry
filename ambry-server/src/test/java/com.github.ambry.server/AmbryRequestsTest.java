...
public class AmbryRequestsTest {
  ...
  public void ttlUpdateTest() throws InterruptedException, IOException, MessageFormatException {
    ...
    BlobId converted =
        new BlobId(blobId.getVersion(), BlobId.BlobIdType.CRAFTED, blobId.getDatacenterId(), blobId.getAccountId(),
            blobId.getContainerId(), blobId.getPartition(), BlobId.isEncrypted(blobId.getID()),
  ...
  private void verifyTtlUpdate(BlobId blobId, long expiresAtMs, long opTimeMs, MessageWriteSet messageWriteSet)
    ...
    StoreKey key = conversionMap.getOrDefault(blobId, blobId);
    ...
    // verify stream
    MessageFormatInputStreamTest.checkTtlUpdateMessage(stream, null, key, blobId.getAccountId(),
        blobId.getContainerId(), opTimeMs, expiresAtMs);

    // verify MessageInfo
    // since the record has been verified, the buffer size before verification is a good indicator of size
    MessageInfoTest.checkGetters(info, key, expectedSize, false, true, expiresAtMs, null, blobId.getAccountId(),
        blobId.getContainerId(), opTimeMs);
  }
